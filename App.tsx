...
export default function App() {
  ...
  const syncStateWithURL = async () => {
    ...
    const knownRoutes = ['landing', 'menu', 'cart', 'qr-verify', 'orders', 'feedback-data', 'feedback', 'super-admin', 'test-supabase', 'accept-invite', 'ai-assistant', 'admin-faq', 'affiliate-auth', 'affiliate-dashboard', 'admin', 'demo', 'articles', 'article', 'sign-up', 'create-restaurant', ...landingOverlays];
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'create-restaurant': return <AdminView menuItems={menuItems} setMenuItems={setMenuItems} categories={categories} setCategories={setCategories} feedbacks={feedbacks} setFeedbacks={setFeedbacks} salesHistory={[]} setSalesHistory={() => {}} adminCreds={{}} setAdminCreds={() => {}} onExit={() => navigateTo('landing')} onLogoUpdate={() => {}} onThemeUpdate={applyTheme} appTheme={appTheme} onOpenFAQ={() => navigateTo('admin-faq')} onBackToMenu={() => navigateTo('landing')} onNavigateToCreateMenu={() => { setActiveSession(null); navigateTo('admin'); }} onNavigateToCreateRestaurant={(userId, email) => navigateTo('create-restaurant')} onLoginSuccess={(restaurantId, restaurantName) => { syncDatabaseData(restaurantId); console.log("Logged in to:", restaurantName); }} isDemo={activeSession?.id?.startsWith('demo-')} />;
