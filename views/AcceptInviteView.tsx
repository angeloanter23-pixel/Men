...
const AcceptInviteView: React.FC<AcceptInviteViewProps> = ({ onComplete, onCancel }) => {
  ...
  useEffect(() => {
    // Correctly parsing hash-based URL params
  ...
  const verifyToken = async (urlToken: string, urlEmail: string) => {
    try {
  ...
  const handleSetup = async (e: React.FormEvent) => {
    ...
    try {
      // Setup the account (backend logic) - fullName removed from parameters
      await MenuService.acceptStaffInvite(token, password);

      // Locally update staff name if we're using a local registry simulation for now
      const staff = JSON.parse(localStorage.getItem('foodie_staff_registry') || '[]');
      const idx = staff.findIndex((s: any) => s.email === email);
      if (idx !== -1) {
        staff[idx].name = fullName;
  ...
  if (step === 'verify') {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-8 font-jakarta">
        ...
        <p className="mt-6 text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">Checking Invite...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-8 font-jakarta text-center space-y-10 animate-fade-in">
        ...
        <div className="space-y-4">
          ...
          <p className="text-slate-500 text-sm font-medium max-w-xs mx-auto leading-relaxed">This invitation is no longer valid or was already used.</p>
        </div>
        <button onClick={onCancel} className="px-12 py-6 bg-slate-900 text-white rounded-[2rem] font-black uppercase text-[10px] tracking-widest shadow-2xl active:scale-95 transition-all">Back to Site</button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FBFBFD] flex flex-col items-center justify-center p-8 font-jakarta animate-fade-in">
      <div className="w-full max-w-sm bg-white p-12 rounded-[4.5rem] shadow-2xl border border-slate-50 space-y-12">
        <header className="text-center space-y-4">
            ...
            <div>
                <p className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.4em] mb-2 leading-none">Team Access</p>
        ...
        <form onSubmit={handleSetup} className="space-y-8">
            ...
            <div className="space-y-2">
                ...
                <input required type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-slate-50 border-none rounded-3xl p-6 text-sm font-black outline-none focus:ring-4 ring-indigo-500/5 shadow-inner" placeholder="••••••••" />
            ...
            <div className="space-y-2">
                ...
                <input required type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full bg-slate-50 border-none rounded-3xl p-6 text-sm font-black outline-none focus:ring-4 ring-indigo-500/5 shadow-inner" placeholder="••••••••" />
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white py-7 rounded-[2.5rem] font-black uppercase text-[10px] tracking-[0.4em] shadow-2xl active:scale-95 transition-all hover:bg-indigo-600 disabled:opacity-50">
              {loading ? <i className="fa-solid fa-spinner animate-spin"></i> : 'Activate Account'}
            </button>
        </form>
      </div>
