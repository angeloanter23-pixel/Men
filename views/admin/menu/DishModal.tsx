        ...
        <header className="px-6 pt-6 pb-4 bg-white border-b border-slate-100 shrink-0">
          ...
          {!isGroupHeader && (
            <div className="bg-[#E8E8ED] p-1 rounded-xl flex border border-slate-100 shadow-inner">
              <button onClick={() => setModalTab('general')} className={`flex-1 py-2.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${modalTab === 'general' ? 'bg-white text-[#007AFF] shadow-sm' : 'text-slate-50'}`}>Information</button>
              <button onClick={() => setModalTab('options')} className={`flex-1 py-2.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${modalTab === 'options' ? 'bg-white text-[#007AFF] shadow-sm' : 'text-slate-50'}`}>Add-ons</button>
            </div>
          )}
        </header>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6">
          {modalTab === 'general' ? (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="space-y-3 pb-4">
                ...
                <button onClick={() => document.getElementById('dish-img-up')?.click()} className="w-full aspect-video bg-white border border-slate-200 rounded-2xl overflow-hidden flex flex-col items-center justify-center gap-3 transition-all hover:border-[#007AFF]/30 group relative shadow-inner">
                  {formData.image ? (
            ...
            <div className="space-y-6 animate-fade-in pb-10">
              <div className="flex justify-between items-center px-2">
                ...
                }} className="bg-[#007AFF] text-white px-4 py-2.5 rounded-xl text-[9px] font-bold uppercase tracking-widest shadow-lg active:scale-95 transition-all">+ New Options</button>
              </div>
          ...
          <div className="pt-2 pb-10">
            <button 
              ...
              className="w-full py-5 bg-slate-900 text-white rounded-none font-black uppercase text-[10px] tracking-[0.3em] shadow-xl active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
            >
