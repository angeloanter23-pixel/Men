    ...
    <div className="space-y-6">
      <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-xl space-y-6">
        <div className="flex justify-between items-center px-2">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Category Hierarchy</h3>
          {!isAddingCat && (
            <button onClick={() => setIsAddingCat(true)} className="bg-indigo-600 text-white px-4 py-2 rounded-xl font-black text-[9px] uppercase shadow-lg shadow-indigo-100 active:scale-95 transition-all">Add New</button>
          )}
        </div>
      ...
      <div className="flex gap-2 bg-slate-100 p-1.5 rounded-2xl">
        <button onClick={() => { setSubTab('add'); setFormData(initialFormState); }} className={`flex-1 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${subTab === 'add' ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-400'}`}>Add Item</button>
        <button onClick={() => setSubTab('list')} className={`flex-1 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${subTab === 'list' ? 'bg-white text-indigo-600 shadow-md' : 'text-slate-400'}`}>Manage List ({items.length})</button>
        {subTab === 'edit' && <button className="flex-1 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest bg-orange-50 text-orange-600 animate-pulse">Editing Mode</button>}
      </div>
      ...
      {(subTab === 'add' || subTab === 'edit') ? (
        <div className="bg-white p-8 rounded-[3.5rem] border border-slate-100 shadow-xl space-y-6 animate-fade-in">
          <div className="flex justify-between items-center px-2">
            <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">{subTab === 'edit' ? 'Update Selection' : 'Create New Selection'}</h4>
          ...
          <button onClick={() => document.getElementById('img-up')?.click()} className="w-full bg-slate-50 border-2 border-dashed border-slate-200 py-12 rounded-[2.5rem] text-[10px] font-black uppercase text-slate-400 flex flex-col items-center gap-3 overflow-hidden hover:border-indigo-300 transition-colors">
            {formData.image ? (
              ...
              <><i className="fa-solid fa-image text-3xl text-slate-200 mb-2"></i><span>Upload High-Res Photo</span></>
            )}
          </button>
          
          <div className="space-y-5">
            <div className="space-y-2">
              <label className="text-[9px] font-black uppercase text-slate-300 ml-4">Item Identity</label>
              <input 
                type="text" 
                placeholder="Dish Name" 
                value={formData.name} 
                onChange={e => setFormData({...formData, name: e.target.value})} 
                className="w-full bg-slate-50 p-5 rounded-2xl font-black text-sm outline-none focus:ring-4 ring-indigo-500/5 transition-all border border-transparent focus:border-indigo-100" 
              />
            </div>
            
            <div className="space-y-2">
              <label className="text-[9px] font-black uppercase text-slate-300 ml-4">Gourmet Description</label>
              <textarea 
                placeholder="Describe the flavors and ingredients..." 
                value={formData.desc} 
                onChange={e => setFormData({...formData, desc: e.target.value})} 
                className="w-full bg-slate-50 p-6 rounded-3xl text-sm h-32 outline-none resize-none focus:ring-4 ring-indigo-500/5 transition-all border border-transparent focus:border-indigo-100" 
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-5">
              ...
              <div className="space-y-2">
                <label className="text-[9px] font-black uppercase text-slate-300 ml-4">Menu Group</label>
                <select 
                  value={formData.cat} 
            ...
            <div className="grid grid-cols-2 gap-5">
              <div className="space-y-2">
                <label className="text-[9px] font-black uppercase text-slate-300 ml-4">Pax Count</label>
                <input 
                    type="text" 
                    placeholder="e.g. 1-2" 
              ...
              <div className="space-y-2">
                <label className="text-[9px] font-black uppercase text-slate-300 ml-4">Wait Time (Mins)</label>
                <input 
                  type="number" 
                  placeholder="15" 
                  value={formData.mins} 
        ...
        <div className="space-y-4 animate-fade-in">
          {items.length === 0 ? (
            <div className="py-24 text-center text-slate-300 font-bold uppercase text-[9px] tracking-[0.3em] border-2 border-dashed border-slate-100 rounded-[3rem] bg-white">
              <i className="fa-solid fa-box-open text-3xl mb-4 opacity-10"></i><br />
              Selection is empty
            </div>
          ) : (
            items.map(item => (
