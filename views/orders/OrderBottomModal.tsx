...
const OrderBottomModal: React.FC<OrderBottomModalProps> = ({ order, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-end justify-center sm:items-center p-0 sm:p-6">
        ...
        <div className="bg-white w-full max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] p-0 shadow-2xl relative animate-slide-up sm:animate-scale-up max-h-[85vh] flex flex-col overflow-hidden pb-10">
            
            <div className="p-8 pb-0 shrink-0 bg-white z-10">
                <div className="w-12 h-1.5 bg-slate-200 rounded-full mx-auto mb-8" />
                <div className="text-center mb-8">
                    <h3 className="text-2xl font-bold text-slate-900 mb-1">{order.item_name}</h3>
                    <p className="text-slate-400 font-medium">Order #{order.id.toString().slice(-4)}</p>
                </div>
            ...
            <div className="flex-1 overflow-y-auto min-h-0 px-8 py-4 pb-12">
                {modalTab === 'summary' ? <OrderItemSummary order={order} /> : <OrderItemStatus order={order} />}
            </div>

            <div className="p-8 pt-4 bg-white border-t border-slate-50 shrink-0 z-10">
